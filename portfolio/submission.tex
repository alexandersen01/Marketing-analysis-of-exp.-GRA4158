...
\newpage
\section{The Return of the Gen-Z Intern}

You are again a data analytics intern at a company that conducted a promotional campaign to
measure the effect of a discount on sales. You can find the corresponding data file on
itslearning.

Unlike in the first randomized experiment (task 1), the treatment in this scenario (receiving the
discount) is not randomized. However, you will be provided with a rich dataset containing 20
covariates (e.g., customer demographics, past purchasing behavior) that should be sufficient to
control for key influences and confounding factors
