...
\begin{abstract}
    ...
    Task~3 estimates the causal effect of a discount on sales from an observational dataset, employing regression adjustment, propensity score matching, Mahalanobis distance matching, and inverse probability weighting under the conditional independence assumption. Task~4 evaluates the impact of a new in-store marketing strategy on weekly product sales using panel data from 21 stores, applying pre-post analysis, two-way fixed effects, difference-in-differences, synthetic control, and synthetic DID to a single treated store.
...
\newpage
\section{Task 4: The sales analyst}
In this task, we evaluate the causal impact of a new in-store marketing strategy on weekly sales of two products at store 109, a single treated store in a multi-store retail chain. A randomized design was not feasible, so we rely on observational panel data covering 21 stores across 104 weeks (2024--2025). The treatment began in week 26 of 2025 (weekind 78). We apply five progressively more sophisticated causal inference methods: pre-post analysis, two-way fixed effects, difference-in-differences, synthetic control, and synthetic DID, and compare their estimates to assess the strategy's effectiveness. All code is provided in Appendix~\ref{app:task_four}.


\subsection{Exploratory Data Analysis}
We start off with some EDA:
\begin{verbatim}
    >  -- p1sales --
    >                 mean    std  count
    > Treated Post                      
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{2026-04-15-11-56-16.png}
\end{figure}
For product 1, we notice that store 109 has much lower sales than control stores (around 320 vs. just over 380 pre-treatment). The levels are very different. Pre-treatment trends look roughly parallel in the plot, both lines are relatively flat with similar fluctuating patterns. Post-treatment, store 109 jumps to around 360 (a roughly +40 unit increase), while control barely moves. This hints at a positive treatment effect for product 1

For product 2, store 109 is yet again below the control group ($\sim 375 $ vs. 460 pre-treatment). Post-treatment, store 109 \textit{drops} to much lower levels, while control is flat. This suggests a possible negative effect on product 2 -- or at least no positive effect. The pre-treatment trends are noisier, and the spikes in store 109 look like promotion-driven jumps, which is worth controlling for. The large level differences are fine for DID (which relies on trends, not levels), but they do motivate synthetic control as a potentially better approach. 
\newpage
\subsection{Pre-Post Analysis (store 109 only)}
This is the simplest analysis, where we just look at store 109 before vs. after, controlling for covariates. It is biased (no counterfactual), but it is a starting point. 
...
\begin{verbatim}
    ...
    > p2price     -4.068014  2.363620  -1.721094  8.523370e-02   -8.700624   
    > p2prom      78.255946  1.972232  39.678879  0.000000e+00   74.390443   
    > compind    122.774150  2.139303  57.389790  0.000000e+00  118.581194   
    >   
    >             0.975]  
    > Intercept  346.904665  
    > post       -25.585247  
    > p2price      0.564597  
    > p2prom      82.121449  
    > compind    126.967107  
    > 
    > Pre-post treatment effect estimate: -29.71 (p=0.0000)
\end{verbatim}
The naive pre-post estimate shows a +33.7 unit increase in weekly sales after treatment (highly significant). Promotions also have a large effect (+35.3), while price has essentially no effect in this single-store sample. The competition index (\verb|compind|) is large but that is because it is on a different scale (0-1). 

The pre-post estimate shows a -29.7 unit decrease; the marketing strategy appears to have hurt product 2 sales. Promotions have a huge effect (+78.3) and price is marginally negative ($p = 0.085$). 

There is one key caveat, and that is that this analysis has no counterfactual. Any time trend common to all stores is absored into the post coefficient. That is why we need DID and synthetic control.
\newpage 
\subsection{Two-Way Fixed Effects Panel Model}
This uses all stores with store and time fixed effects. The \verb|treat\_post| coefficient is the treatment effect, controlling for anything constant within a store (store FE) and anything common across stores in a given week (time FE):
\begin{verbatim}
    > -- p1sales --
...
For product 1, TWFE estimates a treatment effect of +29.98 units ($p< 0.001 $). This is slightly smaller than the naive pre-post (+33.7), which makes sense because the TWFE controls for time-varying shocks common to all stores via week FE. Very high $R^2 $ (0.977) means the FEs and covariates explain almost all variation. Price now shows a significant negative effect (-2.48, $p=0.006 $) since we have cross-store variation to identify it. 

For product 2, TWFE estimates -34.29 units ($p< 0.001 $). The marketing strategy appears to have \textit{reduced} product 2 sales even after controlling for store and time effects. Promotions have a massive effect (+89.42), and price is significantly negative (-2.95). 

With only 21 stores (clusters), cluster-robust SEs can be unreliable. We'll keep this in mind when comparing across methods. The story is consistent so far: positive for P1, negative for P2. Let us commence explicit DID framing. 
\newpage
\subsection{Difference-in-Differences}
First we run a simple $2\times 2 $ DID (collapsed means), then a panel DID regression:
\begin{verbatim}
    ...
    >     post                 2.905      4.167     0.4857
    >     treat_post         -34.354      4.570     0.0000
    >     p2price             -2.881      0.739     0.0001
    >     p2prom              89.915      4.820     0.0000
    > 
    >     DID treatment effect: -34.35 (p=0.0000)
    >     R-squared: 0.9864, N = 2184
\end{verbatim}

The simple $2\times 2 $ DID (raw means) shows a P1 increase of +32.22. Store 109 gained 36.5 units while control gained only 4.3, so the net treatment effect is +32.2. It shows a P2 decrease of -22.26, so store 109 dropped 17.5 while control gained 4.7, so the net effect is -22.3. 

The DID regression with control variates shows a P1 increase of +29.97 ($p<0.001 $), which is very close to the TWFE estimate (29.98), as expected. P2 displays a decrease of -34.35 ($p<0.001 $), which is also very close to the TWFE (-34.29). 

The \verb|post| coefficient is small and insignificant in both regressions (3.5 and 2.9), meaning that there is no strong common time trend beyond what covariates capture, which is a good sign. The \verb|treated| coefficient shows store 109 is structurally lower in sales (-33.6 for P1, -18.4 for P2). 

Notice the raw $2\times 2 $ DID differs from the regression DID because the regression controls for prices and promotions. The regression version is more trustworthy. 

One key concern for DID is that the parallel trends plot showed store 109 is at a very different level from the control mean, and there are only 20 control stores being averaged equally. Some stores may be much better counterfactuals than others. This motivates the synthetic control approach. 

\subsection{Synthetic Control Method}
This constructs a weighted combination of control stores that best matches store 109's pre-treatment sales trajectory. It is ideal here because we have 1 treated unit and a rich donor pool. 

All stores get assigned a donor weight of 0.05 for both P1 and P2 sales. For P1 sales, we get the following results: 
\begin{verbatim}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-14-07-51.png}
\end{figure}

And for P2 sales, we get these results: 
\begin{verbatim}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-14-08-27.png}
\end{figure}

The SCM completely failed here. Every donor got equal weight (1/20 = 0.05). The optimizer just returned the uniform average because it couldn't find \textit{any} weighted combination of control stores that matches store 109's level. The pre-treatment RMSE is enormous (60 and 87), and the plots show the synthetic sits way above the actual.

The reason why is because store 109 has much lower sales than \textit{all} control stores. Since SCM weights are constrained to [0, 1] and sum to 1, the synthetic can never go below the lowest control store. Store 109 is outside the donor pool's range. 

The fix here is a demeaned SCM, as discussed by \textcite{ferman2021synthetic}. We subtract each store's pre-treatment mean, match on the \textit{trajectories/patterns} instead of levels, then add back the treated store's mean:
\newpage
\subsection{Demeaned SCM}

The demeaned SCM performs beautifully: 
\begin{verbatim}
    > ==================================================
    > p1sales
    > ==================================================
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-14-24-42.png}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-14-24-52.png}
\end{figure}
The RMSE drops from 60/87 to 4.05/4.13, and the synthetic now tracks store 109 almost perfectly in the pre-period. The plots confirm this: the blue dashed line hugs the red line closely before week 78. 

Both products lean heavily on stores 110 and 111 ($\sim 87\% $ combined weight for P1, $\sim 79\% $ for P2). These are the stores whose fluctuation patterns most resemble store 109. The remaining weight is spread thinly across a few others. 

P1 has an ATT of +17.66, which is positive but notably smaller than DID (+30.0) and pre-post (+33.7). Every single post-treatment week shows a positive effect (min=+1.3), so the effect is consistent. The SCM suggests a more modest uplift. 

P2 has an ATT of -46.00, which is even more negative than DID (-34.3). Every post-treatment week is negative (max=-1.5). The green gap in the plot is large and persistent. 

The reason why SCM gives different numbers than DID is because DID uses all 20 control stores equally (via the parallel trends assumption). SCM found that stores 110 and 111 are the best counterfactuals. If those stores had different trajectories from the average control store in the post-period, that explains the divergence. The SCM is likely more credible here because it builds a tailored counterfactual rather than assuming all stores trend identically. 

\subsection{Synthetic DID}

SDID \parencite{arkhangelsky2021synthetic} combines the best of both worlds: it reweights both \textit{units} (like SCM) and \textit{time periods} (like DID). This makes it more robust when neither pure parallel trends nor pure SCM fit holds perfectly.
\begin{verbatim}
    > p1sales
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-15-05-51.png}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{2026-04-15-15-06-02.png}
\end{figure}
The intercept is doing its job: \verb|omega_0 = -85.68| (P1) and \verb|-87.17| (P2) absorbs the $\sim 60 - 80 $ unit level gap between store 109 and the control stores. 

For P1 unit weights, store 111 dominates at 0.47, with the rest at around 0.03. This is somehwat concentrated but less extreme than the demeaned SCM (which had stores 110 and 111 at 0.87 combined). The regularization intentionally spreads weight more evenly, and this is by design in SDID: as \verb|zeta| grows, SDID interpolates between SCM (concentrated weights) and DID (uniform weights). 

The P2 unit weights are all nearly uniform (0.03 - 0.12). This is because \verb|sigma_hat| is much higher for P2 (48.99 vs. 22.81) due to those large promotion-driven spikes, which inflates \verb|zeta| to 111.68. High regularization pushes SDID towards DID. This is actually the correct behavior; when the outcome series is noisy, SDID trusts the DID averaging more than the SCM matching. 

For P1, 38\% of the weights is on the last 10 pre-periods, meaning SDID upweights recent history. For P2, only 23\%, which again is more regularized due to noise.

The demeaned SCM implementation produced the best pre-treatment fit (RMSE $\sim 4 $). However, one thing worth noting is that we matched only on \textit{outcome trajectories}, not on covariates. The classical Abadie et al. approach also matches on predictor variables. Since our fit is already excellent, adding covariates would not change much. 

\subsection{Which methods should we trust most?}

The pre-post analysis (Section 4.1) is the weakest. There is no counterfactual, so any common time trend gets baked into the estimate. It is useful only as a baseline. DID/TWFE (Sections 4.2 and 4.3) are the workhorses. The DID regression and TWFE give nearly identical estimates ($\sim +30 / -34$) because they are algebraically equivalent when you include store + time FE. The parallel trends assumption is somewhat supported by the pre-treatment plots (both P1 and P2 show roughly parallel movement), but the \textit{level gap} between store 109 and the control mean is large. DID assumes all 20 control stores contribute equally to the counterfactual, which is crude. 

The standard SCM (Section 4.4) failed due to the convex hull problem, but the demeaned SCM (Section 4.5) is arguably the most credible method for this setting. With only 1 treated unit, SCM's strength, which is building a tailored counterfactual from the best-matching donors, is exactly what we need. The pre-treatment fit was excellent (RMSE $\sim 4$), and the weights concentrated on stores 110 and 111, which track store 109's fluctuation patterns closely. The lower estimates (+18 for P1, $\sim -46$ for P2) suggest that the DID methods may be biased because the average control store is not a good counterfactual for store 109. 

SDID (Section 4.6) sits between DID and SCM by design. For P1, the weights concentrated on store 111 (similar to SCM), giving +19.5. For P2, high noise from promotion spikes inflated the regularization, pushing weights toward uniform (i.e., towards DID), giving $-36.9$. SDID's advantage is its double-robustness: it is consistent if \textit{either} the parallel trends or the SCM fit holds. 

Therefore we would recommend emphasizing the SCM and SDID estimates as the primary findings, while keeping DID as a robustness check. The SCM pre-treatment fit provides strong visual evidence of counterfactual validity. 

There are some challenges and limitations to discuss. There is only one single treated unit, where all inference is based on one store. There is no way to assess treatment effect heterogeneity across stores. There are only 21 clusters for cluster-robust SEs in DID. Standard errors may be unreliable. SCM has no standard inference; we could run placebo tests (apply SCM to each control store pretending it's treated), but we did not. The post-period is relatively short at just 27 weeks, so it is hard to know if the effect is actually persistent. The promotion spikes in P2 create noise that complicates SCM matching and inflates SDID regularization. There are no spillovers assumed, so if the marketing strategy in store 109 affected nearby stores, the control group is contaminated, which is a SUTVA violation. Finally, store 109 is an outlier in sales levels. It is already well below all control stores, raising questions about external validity. 

Our recommendations to management, using the SCM/SDID estimates (which we argue are the most credible), is that the strategy lifts P1 sales by approximately +18 to +20 units per week. However, it also reduces P2 sales by approximately -37 to -46 units per week, so the net effect is negative. Before rolling out chain-wide, management should investigate \textit{why} P2 declines and whether the strategy can be modified to avoid cannibalization. A proper randomized trial across multiple stores would provide definitive evidence with valid statistical inference. 
